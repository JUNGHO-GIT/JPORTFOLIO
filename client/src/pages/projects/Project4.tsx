...
export const Project4 = memo(() => {
	...
	const [images, _setImages] = useState<string[]>([`project4_1`, `project4_2`, `project4_3`, `project4_4`, `project4_5`]);

	// 2-3. useEffect --------------------------------------------------------------------------------
	useEffect(() => {
		setLOADING(true);
		axios
		.get(`${URL}${PROJECTS_URL}/detail`, {
			params: {
				project_id: `Project4`,
			},
		})
	...
	const projectNode = () => (
		<Paper className={`detail-wrapper ${paperClass} border-0 radius-0 shadow-0`}>
					...
					>
						{images.map((image: string, i: number) => (
							<SwiperSlide className={`w-100p p-relative`} key={i}>
								<Div className={`p-5px`}>
									<Img max={1000} hover={false} shadow={true} radius={true} border={true} src={`${image}.webp`} group={`projects`} />
								</Div>
							</SwiperSlide>
						))}
			...
			<Br m={40} />
			<Grid container={true} spacing={2} columnSpacing={10} className={`h-100p d-top border-1 radius-2 shadow-1 px-4vw py-4vh`}>
				{/** info, features * */}
				<Grid size={xxs || xs || sm ? 12 : md || lg || xl || xxl ? 6 : 6}>
					<Grid container={true} spacing={2}>
						{OBJECT?.project_section
						?.filter((_: any, f: number) => f < 2)
								...
								{section.project_section_contents.map((contents: any, j: number) => (
									<Div className={`w-100p d-col-left mb-30px`} key={j}>
										<Div className={`d-row-center mb-10px`}>
											<Div className={`fs-0-4rem fw-300 dark-navy mr-5px`}>{`●`}</Div>
											<Div className={`fs-1-1rem fw-600 dark-navy`}>{contents.project_contents_title}</Div>
										</Div>
										<Div className={`d-row-center ml-10px`}>
											{contents.project_contents_detail.map((detail: any, k: number) => (contents.project_contents_title === `Url` || contents.project_contents_title === `Git1` || contents.project_contents_title === `Git2` ? (
														...
														<Div className={`d-row-left mr-0px`} key={k}>
															<Div key={k} className={`fs-1-0rem fw-400 light-black lh-2-0`} onClick={() => {}}>
									...
									<Div className={`w-100p d-col-left mb-30px`} key={j}>
										...
										<Grid container={true} spacing={0} rowSpacing={2} className={`d-left ml-1vw`}>
											{contents.project_contents_detail.map((detail: any, k: number) => (
												<Grid size={xxs ? 6 : xs ? 4 : sm ? 3 : md ? 4 : lg ? 4 : xl ? 3 : xxl ? 3 : 3} className={`d-row-left`} key={k}>
													<Img max={20} hover={true} shadow={false} border={false} radius={false} group={`icons`} src={`${detail.project_detail_icon}.webp`} className={`mr-5px`} />
													<Div className={`fs-1-0rem fw-400 light-black lh-2-0`}>{detail.project_detail_value}</Div>
												</Grid>
											))}
										</Grid>
									</Div>
